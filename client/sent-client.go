...
type TritonSentClient interface {
	GetSents(texts []string) []dto.SentResult
	IsReady() bool
}

type tritonSentClient struct {
	grpcClient   triton.GRPCInferenceServiceClient
	modelName    string
	modelVersion string
}

func CreateSentClient(serverUrl string, modelName string, modelVersion string) TritonSentClient {
	...
	sentClient := &tritonSentClient{
		grpcClient:   grpcClient,
		modelName:    modelName,
		modelVersion: modelVersion,
	}

	liveResponse := sentClient.ServerLive()
	if !liveResponse.Live {
		panic("server not live")
	}
	readyRepsonse := sentClient.ServerReady()
	if !readyRepsonse.Ready {
		panic("server not ready")
	}
	metadataResponse := sentClient.ModelMetadata(modelName, modelVersion)
	log.Printf("get model metadata: %v", metadataResponse)

	return sentClient
}

func (c *tritonSentClient) Preprocess(inputs []string) []byte {
	var inputBytes []byte
...
func (c *tritonSentClient) Postprocess(response *triton.ModelInferResponse) []dto.SentResult {
	var sent_texts []string
	var sent_indices []int64
	var group_indices []int64
	for i, output := range response.Outputs {
		rawOutput := response.RawOutputContents[i]
		if output.Name == "sent_text" {
			offset := 0
			for offset < len(rawOutput) {
				length := int(binary.LittleEndian.Uint32(rawOutput[offset : offset+4]))
				offset += 4
				sent_texts = append(sent_texts, string(rawOutput[offset:offset+length]))
				offset += length
			}
		}
		if output.Name == "sent_index" {
			s := make([]int64, output.Shape[0])
			buf := bytes.NewReader(rawOutput)
			binary.Read(buf, binary.LittleEndian, &s)
			sent_indices = append(sent_indices, s...)
		}
		if output.Name == "group_index" {
			s := make([]int64, output.Shape[0])
			buf := bytes.NewReader(rawOutput)
			binary.Read(buf, binary.LittleEndian, &s)
			group_indices = append(group_indices, s...)
		}
	}

	if sent_texts == nil || sent_indices == nil || group_indices == nil {
		panic("fail to post-process: results are `nil`")
	}
	if len(sent_texts) != len(sent_indices) || len(sent_texts) != len(group_indices) {
		panic("fail to post-process: result lengths are not equal")
	}

	results := []dto.SentResult{}
	for i := 0; i < len(sent_texts); i++ {
		result := dto.SentResult{
			Text:       sent_texts[i],
			SentIndex:  sent_indices[i],
			GroupIndex: group_indices[i],
		}
...
func (c *tritonSentClient) ServerLive() *triton.ServerLiveResponse {
	...
	if err != nil {
		panic("fail to get server live result 1")
	}
	return serverLiveResponse
}

func (c *tritonSentClient) ServerReady() *triton.ServerReadyResponse {
	// Create context for our request with 10 second timeout
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()
...
func (c *tritonSentClient) ModelMetadata(modelName string, modelVersion string) *triton.ModelMetadataResponse {
	// Create context for our request with 10 second timeout
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()
...
func (c *tritonSentClient) ModelInfer(rawInput []byte, modelName string, modelVersion string) *triton.ModelInferResponse {
	// Create context for our request with 10 second timeout
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()
...
func (c *tritonSentClient) GetSents(texts []string) []dto.SentResult {
	fmt.Println(texts)
...
func (c *tritonSentClient) IsReady() bool {
